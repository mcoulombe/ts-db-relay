...
# Set ownership of data and cert directories
chown -R mongodb:mongodb /data/db /etc/ssl/mongodb

# CA certificate is already in the shared volume due to volume mount
# /etc/ssl/mongodb is mounted to ../data/mongodb-certs
chmod 644 /etc/ssl/mongodb/ca.crt

# Start database with TLS as mongodb user
...
until mongosh --tls --tlsCAFile /etc/ssl/mongodb/ca.crt --tlsAllowInvalidHostnames \
...
mongosh --tls --tlsCAFile /etc/ssl/mongodb/ca.crt --tlsAllowInvalidHostnames admin --eval "
try {
  db.createUser({
...
# Create/update test role
echo "Creating test role..."
mongosh --tls --tlsCAFile /etc/ssl/mongodb/ca.crt --tlsAllowInvalidHostnames "$MONGODB_DB" --eval "
try {
  db.createRole({
    role: '$MONGODB_USER',
    privileges: [
      { resource: { db: '$MONGODB_DB', collection: '' }, actions: [ 'find', 'insert', 'update', 'remove', 'createCollection', 'createIndex', 'dropCollection' ] }
    ],
    roles: []
  });
} catch (e) {
...
try {
  db.createUser({
    ...
    roles: [ { role: '$MONGODB_USER', db: '$MONGODB_DB' } ]
  });
} catch (e) {
  if (e.code === 51003) {
    db.updateUser('$MONGODB_TEST_USER', { pwd: '$MONGODB_TEST_PASSWORD', roles: [ { role: '$MONGODB_USER', db: '$MONGODB_DB' } ] });
  } else {
    throw e;
  }
}" > /dev/null

echo "Database '$MONGODB_DB' setup complete."
